...
public class DeepQNetwork {
    ...
    DeepQNetwork(MultiLayerConfiguration conf, int replayMemoryCapacity, float discount, double epsilon, int batchSize, int updateFreq, int replayStartSize, int inputLength, int numActions, int rngSeed){
        ...
        r = rngSeed == 0 ? new Random() : new Random(rngSeed);
        BatchSize = batchSize;
...
//Then sample a mini batch of experiences from the memory randomly, and update our Q function on that, similar to mini-batch gradient descent.
    ...
    int getAction(INDArray Inputs , int ActionMask[]){
        ...
        if(Epsilon > r.nextDouble()) {
            LastAction = r.nextInt((int) outputs.size(1)); //I made it int
            while(ActionMask[LastAction] == 0)
                LastAction = r.nextInt((int) outputs.size(1));
            System.out.println("Random a: " + LastAction);
            return LastAction;
        }
    ...
    void observeReward(float Reward , INDArray NextInputs , int NextActionMask[]){
        ...
        UpdateCounter++;
        System.out.println(" Counter: " + UpdateCounter);
